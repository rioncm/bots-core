...
def parse_edi_file(**ta_info):
    ...
    try:
        ...
        content = core_text.get_relevant_text_for_UnicodeError(exc)
...
class Inmessage(message.Message):
    ...
    def messagegrammarread(self, typeofgrammarfile):
        ...
        core_text.updateunlessset(self.ta_info, self.defmessage.syntax)
            ...
            if SUBTRANSLATION in structure_level[structure_index]:
                ...
                try:
                    ...
                    if hasattr(self.defmessage.module, 'getmessagetype'):
                        messagetype2 = core_scripts.runscript(
    ...
    def _readcontent_edifile(self):
        ...
        self.rawinput = core_io.readdata(
...
class fixed(Inmessage):
    ...
    def _readcontent_edifile(self):
        """open the edi file."""
        botsglobal.logger.debug('Read edi file "%(filename)s".', self.ta_info)
        self.filehandler = core_io.opendata(
            filename=self.ta_info['filename'],
            mode="r",
            charset=self.ta_info['charset'],
            errors=self.ta_info['checkcharsetin'],
...
class excel(csv):
    def initfromfile(self):
        ...
        try:
            self.xlrd = core_imports.botsbaseimport('xlrd')
        ...
        infilename = core_paths.abspathdata(self.ta_info['filename'])
        try:
...
class edifact(var):
    ...
    def _readcontent_edifile(self):
        ...
        botsglobal.logger.debug('Read edi file "%(filename)s".', self.ta_info)
        # read as binary
        self.rawinput = core_io.readdata_bin(filename=self.ta_info['filename'])
    ...
    def handleconfirm(self, ta_fromfile, routedict, error):
        ...
        if not core_routing.globalcheckconfirmrules(confirmtype):
            return
        editype = 'edifact'
        ...
        for UNB in self.getloop({'BOTSID': 'UNB'}):
            ...
            for nodeunh in UNB.getloop({'BOTSID': 'UNB'}, {'BOTSID': 'UNH'}):
                ...
                if messagetype[:6] in ['CONTRL', 'APERAK'] or not core_routing.checkconfirmrules(
                        confirmtype,
            ...
            tscript, _toeditype, tomessagetype = core_routing.lookup_translation(
                fromeditype=editype,
            ...
            if not tscript:
                ...
                translationscript, scriptfilename = core_imports.botsimport("mappings", editype, tscript)
            # generate CONTRL-message.
            # One received interchange->one CONTRL-message
            reference = str(unique_counter("messagecounter"))
    ...
    def handleconfirm(self, ta_fromfile, routedict, error):
        ...
        for GS in self.getloop({'BOTSID': 'ISA'}, {'BOTSID': 'GS'}):
            ...
            for nodest in GS.getloop({'BOTSID': 'GS'}, {'BOTSID': 'ST'}):
                if core_routing.checkconfirmrules(
                        confirmtype,
                        idroute=self.ta_info['idroute'],
                        idchannel=self.ta_info['fromchannel'],
                        frompartner=sender,
            ...
            reference = str(unique_counter("messagecounter")).zfill(4)
            ta_confirmation = ta_fromfile.copyta(status=TRANSLATED)
            filename = str(ta_confirmation.idta)
            # make outmessage object
            out = outmessage.outmessage_init(
...
class xml(Inmessage):
    ...
    def initfromfile(self):
        botsglobal.logger.debug('Read edi file "%(filename)s".', self.ta_info)
        filename = core_paths.abspathdata(self.ta_info['filename'])
        ...
        if self.ta_info['messagetype'] == 'mailbag':
            ...
            try:
                module, _grammarname = core_imports.botsimport("grammars", "xml", "mailbag")
...
class db(Inmessage):
    ...
    def initfromfile(self):
        botsglobal.logger.debug('Read edi file "%(filename)s".', self.ta_info)
        self.root = core_io.readdata_pickled(filename=self.ta_info['filename'])
...
class raw(Inmessage):
    ...
    def initfromfile(self):
        botsglobal.logger.debug('Read edi file "%(filename)s".', self.ta_info)
        self.root = core_io.readdata_bin(filename=self.ta_info['filename'])
