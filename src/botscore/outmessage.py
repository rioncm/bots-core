...
class Outmessage(message.Message):
    ...
    def _initwrite(self):
        ...
        self._outstream = core_io.opendata(
...
class xml(Outmessage):
    ...
    def _xmlcorewrite(self, xmltree, root):
        ...
        if self.ta_info['indented']:
            if isinstance(self.ta_info['indented'], str):
                xmlutil.indent_xml(root, indentstring=self.ta_info['indented'])
            else:
                xmlutil.indent_xml(root)
    ...
    def _initwrite(self):
        botsglobal.logger.debug('Start writing to file "%(filename)s".', self.ta_info)
        self._outstream = core_io.opendata_bin(self.ta_info['filename'], 'wb')
...
class templatehtml(Outmessage):
    ...
    def __init__(self, ta_info):
        ...
        if self.template_engine == 'django':
            self.template = core_imports.botsbaseimport('django.template')
    ...
    def _write(self, node_instance):
        templatefile = core_paths.abspath(self.__class__.__name__, self.ta_info['template'])
        try:
            botsglobal.logger.debug('Start writing to file "%(filename)s".', self.ta_info)
            if self.template_engine == 'django':
        ...
        try:
            ...
            # genshi template render
                ...
                with core_io.opendata_bin(self.ta_info['filename'], 'wb') as filehandler:
...
class db(Outmessage):
    ...
    def writeall(self):
        if self.root is None:
            ...
            raise OutMessageError(_("No outgoing message"))
        botsglobal.logger.debug('Start writing to file "%(filename)s".', self.ta_info)
        core_io.writedata_pickled(self.ta_info['filename'], self.root)
        botsglobal.logger.debug('End writing to file "%(filename)s".', self.ta_info)
